...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($icon)) {
        ...
        if (file_put_contents('../php/config.php', $configContent)) {
            header('Location: categories.php?success=1&message=分类更新成功！');
            exit;
        } else {
                ...
                <form action="category-edit.php?id=<?php echo $categoryId; ?>" method="POST">
                    <div class="form-row">
                        <div class="form-group">
                            <label for="id" class="form-label">分类ID</label>
                            <input type="text" id="id" name="id" class="form-input" value="<?php echo $category['id']; ?>" readonly style="background-color: #f5f5f5; cursor: not-allowed;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name" class="form-input" value="<?php echo $category['name']; ?>" placeholder="例如：开发工具，数据分析" required>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="icon" name="icon" class="form-input" value="<?php echo $category['icon']; ?>" placeholder="例如：⚙️, 📊, 🎨" required>
                            <div class="form-hint" style="font-size: 12px; color: #999; margin-top: 4px;">
                                支持Emoji或图标代码
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="status" name="status" class="form-input" required>
                                <option value="active" <?php echo $category['status'] === 'active' ? 'selected' : ''; ?>>启用</option>
                                <option value="inactive" <?php echo $category['status'] === 'inactive' ? 'selected' : ''; ?>>禁用</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" class="form-textarea" placeholder="请输入分类描述" rows="3"><?php echo $category['description'] ?? ''; ?></textarea>
                    </div>
